...
import cn.scorestatistics.demo.model.pojo.IpInfo;
import cn.scorestatistics.demo.model.pojo.IpWeatherResult;
...
public class IPInfoUtil {
     ...
     * @return
     */
    public static String getIpCityByPut(String ip) {
        if(ip != null) {
            String url = GET_WEATHER + ip;
            String json = HttpUtil.sendPut(url);
            String result = "未知";
            try{
                IpWeatherResult weather = new Gson().fromJson(json, IpWeatherResult.class);
                result = weather.getResult().get(0).getCity()+" "+weather.getResult().get(0).getDistrct();
